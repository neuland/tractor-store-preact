...
export default function createApp(beforeRoutes = (app) => {}) {
  ...
  // API endpoint
  app.get("/checkout/api/about", (c) => c.json(aboutData));
  app.get("/checkout/api/navigation", (c) => {
    const min = parseInt(c.req.query("min"), 10);
    const max = parseInt(c.req.query("max"), 10);
    return c.json({ avg: (min + max) / 2 });
  });

  app.get("/checkout/esi/navigation", async (c) => {
    const params = {
  ...
  app.get("/checkout/:page?", async (c) => {
    ...
    <body>
      ...
      <script type="application/json" data-state>${state}</script>
      <script src="/checkout/static/client.js"></script>
    </body>
    </html>
  `);
  });
